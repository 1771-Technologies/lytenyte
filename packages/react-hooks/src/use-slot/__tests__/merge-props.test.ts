...
describe("mergeProps", () => {
  test("child props override slot props by default", () => {
  ...
  test("composes event handlers if both exist", () => {
  ...
  test("uses slot handler if only slot has it", () => {
    const slotClick = vi.fn();
  ...
  test("uses child handler if only child has it", () => {
  ...
  test("ignores undefined className and style", () => {
  ...
  test("handles non-handler props normally", () => {
