...
const DEFAULTS = {
  move: { duration: 320, easing: "ease" },
  enter: { duration: 220, easing: "ease" },
  exit: { duration: 180, easing: "ease" },
  ...
  useLayoutEffect(() => {
    ...
    for (const row of layout.center) {
      ...
      if (prev === undefined) {
        ...
        const t = el.animate([{ transform: "translateY(-6px)" }, { transform: "translateY(0px)" }], {
        ...
        void o.finished.then(() => moveEnterCleanupRef.current.delete(data));
      } else {
        // ── MOVE ─────────────────────────────────────────────────────────────
        if (moveConf === false) continue;

        const delta = prev.y - yPositions[row.rowIndex];
        if (delta === 0) continue;
        // Skip rows that jumped more than 1.5 viewports — they were off-screen.
        if (Math.abs(delta) > viewportHeight * 1.5) continue;
        ...
        void anim.finished.then(() => moveEnterCleanupRef.current.delete(data));
      }
    }
    ...
    for (const data of prevDataSet) {
      ...
      void anim.finished.then(remove);
    }
