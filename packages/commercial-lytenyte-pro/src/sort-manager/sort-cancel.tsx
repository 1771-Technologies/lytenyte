...
export const SortCancel = forwardRef<HTMLDivElement, JSX.IntrinsicElements["button"] & SortCancelProps>(
  function SortCancel({ as, ...props }, forwarded) {
    const ctx = useSortManagerCtx();

    const onCancel = useEvent(() => {
      const model = ctx.mode ? ctx.grid.state.columnPivotModel.get().sorts : ctx.grid.state.sortModel.get();
