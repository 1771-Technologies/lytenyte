...
export const RowsContainer = memo(
  forwardRef<HTMLDivElement, JSX.IntrinsicElements["div"]>(function Rows(props, forwarded) {
    const { id, columnMeta, xPositions, yPositions, vpInnerWidth, vpInnerHeight } = useGridRoot();

    const startWidth = xPositions[columnMeta.columnVisibleStartCount];
    const endWidth = xPositions.at(-1)! - xPositions.at(-1 - columnMeta.columnVisibleEndCount)!;
