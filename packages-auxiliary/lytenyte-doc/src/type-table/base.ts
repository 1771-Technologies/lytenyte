...
import { createProject, type TypescriptConfig } from "./create-project.js";
...
import { getSimpleForm } from "./get-simple-form.js";
...
export function createGenerator(config?: GeneratorOptions | Project) {
  ...
  return {
    generateDocumentation(
      ...
      const content = file.content ?? fs.readFileSync(path.resolve(file.path)).toString();
...
function getDocEntry(prop: TsSymbol, context: EntryContext): DocEntry | undefined {
  ...
  const entry: DocEntry = {
    ...
    tags,
    type,
    simplifiedType: getSimpleForm(subType, program.getTypeChecker(), isOptional),
    required: !isOptional,
