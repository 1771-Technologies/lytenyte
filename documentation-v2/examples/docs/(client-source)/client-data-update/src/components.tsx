...
export function PercentCellPositiveNegative({ api, column, row }: Grid.T.CellRendererParams<GridSpec>) {
  ...
  return (
    <div className={clsx("flex h-full w-full items-center justify-end gap-2 tabular-nums")}>
      {diff !== 0 && (
        <div
          className={clsx(
      ...
      <span
        className={tw(field < 0 ? "text-red-600 dark:text-red-300" : "text-green-600 dark:text-green-300")}
      >
        {value}
      </span>
    </div>
