...
export function HeaderCell({ column, api }: Grid.T.HeaderParams<GridSpec>) {
  const isUpdating = api.editing.useValue((x) => x === column.id);
  ...
  return (
    <div
      ...
      onDoubleClick={() => {
        ...
        api.editing.set(column.id);
      }}
    >
      {isUpdating && (
        <HeaderCellInput
          name={column.name ?? column.id}
          onAccept={(name) => api.updateHeaderName(name, column.id)}
        />
      )}
...
function HeaderCellInput({
  ...
  onAccept: onUpdate,
  ...props
}: JSX.IntrinsicElements["input"] & { name: string; onAccept: (newName: string) => void }) {
  const [value, setValue] = useState(name);

  return (
    <input
      className="bg-ln-gray-10 h-full w-full px-2 outline-none"
      value={value}
