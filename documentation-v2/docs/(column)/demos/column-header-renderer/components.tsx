...
const idToIcon = {
  ...
  customer: SquareUserIcon,
  purchaseDate: CalendarFoldIcon,
  paymentMethod: CreditCardIcon,
  email: AtSignIcon,
};

export function HeaderWithIcon({ column }: Grid.T.HeaderParams<GridSpec>) {
  const Icon = idToIcon[column.id as keyof typeof idToIcon];

  return (
    <div className={tw("flex h-full w-full items-center gap-1", column.type === "number" && "justify-end")}>
