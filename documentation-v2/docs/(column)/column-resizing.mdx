...
```tsx
<Grid columnBase={{ resizable: true }} />
```
...
::demo[Resizing Columns="./demos/column-resizing-drag"]
...
```ts
api.columnResize({ exchange: 200, network: 100 });
```
...
```ts
api.columnUpdate({ exchange: { width: 200 }, network: { width: 100 } });
```
...
::demo[Column Width Bounds="./demos/column-resizing-minmax"]

The column `widthMin` and `widthMax` properties define visual layout bounds that
LyteNyte Grid enforces. LyteNyte Grid does not clamp the column's `width` value to
those limits, so your code can set `width` outside `widthMin`/`widthMax`. If `width` is out
of bounds and you later change `widthMin` or `widthMax`, LyteNyte Grid immediately
...
::demo[Full Width Viewport Filling="./demos/column-resizing-width-flex"]
...
::demo[Size-to-Fit Columns="./demos/column-resizing-size-to-fit"]
...
::demo[Truncating and Hashing="./demos/column-resizing-measure-text"]
