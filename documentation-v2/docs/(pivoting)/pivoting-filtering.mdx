...
property accepts an array of [`HavingFilterFn`](/docs/reference/grid-types#filter) functions.

The demo below demonstrates having filter functions. You can update the filter applied
to the pivot column by clicking the funnel icon in the column header.

Having filters are applied to rows at a specific depth; in this case, the child
rows of countries will be filtered. Refer to the
...
::demo[Filter Before Pivot="./demos/pivot-existing-filters"]
