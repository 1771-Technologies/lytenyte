...
export default function PivotDemo() {
  const [pivots, setPivots] = useState<PillManager.T.PillItem[]>(() =>
    columns
  ...
  const ds = useClientDataSource<GridSpec>({
    ...
    pivotMode: true,
    pivotModel: {
      columns: pivots.filter((x) => x.active),
      measures: [
        {
  ...
  const pillRows = useMemo(() => {
    ...
    return [{ id: "column-pivots", label: "Column Pivots", type: "column-pivots", pills: pivotPills }];
  }, [pivots]);
    ...
    <>
      <div>
        <PillManager
          ...
          onPillRowChange={(ev) => {
            const activeFirst = ev.changed[0].pills.filter((x) => x.active);
            const nonActive = ev.changed[0].pills.filter((x) => !x.active);
            setPivots([...activeFirst, ...nonActive]);
          }}
        />
      </div>
