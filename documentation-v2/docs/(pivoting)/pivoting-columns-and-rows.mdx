...
Since pivots are a rotation of the grid view, you can seamlessly swap row and column pivots. The demo
above does this when you click the **Swap** button.

The demo used the [Pill Manager component](/docs/component-pill-manager) to configure pivots. The pivot
state is kept in React state and updated whenever the pills are clicked or dragged. The pivot state is
then mapped onto the client data source for the final view configuration, as shown in the code blow.
