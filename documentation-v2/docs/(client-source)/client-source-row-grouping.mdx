...
LyteNyte Grid's client source uses the `group` property to create a hierarchical row
structure. When grouping is enabled, the source creates two row types: leaf rows and
group rows. If you are unfamiliar with the different row node types in LyteNyte Grid,
see the [Row Overview guide](/docs/row-overview).

:::note

This guide covers the grouping functionality available in LyteNyte Grid for the
client row source. Grouping goes hand-in-hand with row aggregations. We highly recommend
reading the [Client Row Aggregations guide](/docs/client-source-aggregations)
after you finish reading this guide.

:::

## Grouping Function

Provide the `useClientDataSource` hook with a function that returns an array of path
values to group by. A path value is any `string` value or `null`.
...
The demo below demonstrates a non-uniform group. Notice that any row with `"Secondary"` education
is not grouped, and any row with **Marital** status equal to **"Single"** is only grouped by **Job**.

::demo[Non-Uniform Groups="./demos/client-row-grouping-fn-non-uniform"]

:::tip

If you are coming to LyteNyte Grid from other another data grid, you may have seen non-uniform groups
referred to as **Tree Data**. In LyteNyte Grid, this path-based array of values is simply
a non-uniform grouping of rows.

LyteNyte Grid provides a [tree data source](/docs/tree-source-overview). However,
this source is intended for use with object data, not array data.
...
This means a dimension is any object with a `field` or `id` property. All valid LyteNyte Grid
columns conform to the `Dimension` type and may be used as dimensions. The demo below demonstrates
using dimensions to group rows:
...
Updating row groups is as simple as updating the value of the `group` property. The demo below demonstrates
this behavior by updating groups using the [Pill Manager component](/docs/component-pill-manager).
...
expand and collapse rows. The demo below demonstrates this.
...
::demo[Sibling Expansion="./demos/client-row-grouping-group-siblings"]
