...
pro: true
description: Manage selection and order through LyteNyte Grid's Pill Manager component. The Pill Manager
  component is a versatile item list component with drag-and-drop support.
---

LyteNyte Grid exports the `PillManager` component. `PillManager` is a headless component composed of
separate parts that you can optionally assemble for manual customization or render using a
...
Every `PillItemSpec` in a `PillRowSpec` must have the `active` state property set. The
`active` property determines whether a pill is enabled. What it means for a pill to be
active depends on your use case.

For example, you can use the `active` property to determine column visibility, as the demo
below demonstrates. Pills that are not active are dimmed.

::demo[Pill Active State="../(column)/demos/column-visibility"]

When the `active` state of a pill changes, the `PillManager` calls the `onPillRowChange`
handler. The interface for `onPillRowChange` is shown below. You can use the provided
parameters to update your pill row state.
...
The `PillManager` is a headless component. All parts are rendered using a render prop
provided to the `PillManager`. The render prop is optional. If you do not provide it,
the `PillManager` uses a default implementation.
...
Set the `orientation` property on the `PillManager` to `"vertical"` to orient pills
vertically. The demo below demonstrates this configuration. Because the `PillManager`
is headless, you must apply the appropriate styles when rendering a vertical layout.

::demo[Vertical Pill="./demos/pill-manager-vertical"]

## Pill Manager Styling

The `PillManager` component is headless and unstyled by default. You can import
pre-made styles from the LyteNyte Grid package to match the `PillManager` theme
with existing LyteNyte Grid styles.

```ts
import "@1771technologies/lytenyte-pro-experimental/pill-manager.css";
```

The pre-made styles also provide pill theming for each `PillRowSpec` type.
