...
description: Create a filter that includes or excludes rows based
  on whether a cell value appears in a set.
---
...
Type a filter query to select product Name values to display.
The grid shows only rows that match the selected values.
...
[Tree View](/docs/component-tree-view) wraps LyteNyte Grid, so it uses the same row selection model.
For details, see the [Row Selection guide](/docs/row-selection).
