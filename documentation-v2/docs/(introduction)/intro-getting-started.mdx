...
step: Quickly start with LyteNyte Grid and see it in action.
description:
...
3. Create a `GridSpec` type. LyteNyte Grid uses TypeScript for code completion and type checking.
   ...
   and pass it as the type parameter to the relevant grid types. The specification interface can
   do more than define the row data type, and this guide covers that later. For best practices regarding TypeScript usage
   and LyteNyte Grid, see the [TypeScript guide](/docs/prodready-typescript).
...
values, you must provide a custom cell renderer. You will do that next.
...
cell-specific properties, defined by the [CellRendererParams](/docs/reference/cells#cellrendererparams)
type.
...
any React content you want. Focus on the function definition. You define a cell renderer by creating a React component
that accepts `CellRendererParams`. This example also uses the `GridSpec` type, which we defined earlier, to improve type checking.
...
We use Tailwind CSS to style components. LyteNyte Grid has no opinion on which styling framework you use.
If you follow this guide line by line and want the cell renderers to render correctly, set up Tailwind by following
the [Tailwind installation guide](https://tailwindcss.com/docs/installation/using-vite), and add applying our
[Tailwind theme configuration (todo)](/theme).
...
Finally, pass a sort definition to the client row data source. Create the sort definition by deriving a value
from the column state. The code below builds a `sort` value from the column that has a `sort` value.
...
[Row Master Detail guide](/docs/row-detai).
...
- [Columns](/docs/columns): Review all column configuration options.
- [Grid Theming](/docs/grid-theming): Learn different approaches to theming LyteNyte Grid.
- [Grid Reactivity](/docs/grid-reactivity): Learn how LyteNyte Grid enables declarative reactivity.
- [Client Row Data Source](/docs/reference): Learn how to work with client-side row data.
