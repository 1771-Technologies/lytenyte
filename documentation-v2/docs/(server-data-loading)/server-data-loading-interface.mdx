...
may be found in the [API reference](/docs/reference/grid-api). In plain
...
- A data response for scrollable rows, described in this
  [API reference](/docs/reference/grid-api).
...
The `id` value maps to the [row node's](/docs/reference/rows) `id` property, and the `data`
...
items to create [row group nodes](/docs/reference/rows). Group nodes represent branches in
...
[row group node](/docs/reference/rows) is intentional. The server response is designed to
