...
The `id` value maps to the [row node's](/docs/reference/row) `id` property, and the `data`
...
items to create [row group nodes](/docs/reference/row). Group nodes represent branches in
...
[row group node](/docs/reference/row) is intentional. The server response is designed to
