...
LyteNyte Grid's `useServerDataSource` hook accepts a callback for the `onRowDataChange` property.
The edit handler is called when a cell edit occurs in the grid and is responsible for sending the update
...
The example below demonstrates this. You can edit any cell value (by double clicking on the cell);
after committing the change, it won't update immediately but will refresh shortly to show the new value.
This behavior isn't ideal, which is why the next section explains how to optimistically update the
client value so changes appear instantly.
...
server update will succeed. The `rowUpdateOptimistically` property makes cell updates apply
instantly on the client, but remember that this change is client only. Developers
must still implement the `onRowDataChange` function and handle any errors
that occur when sending updates to the server.
