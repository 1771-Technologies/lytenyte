...
syntax highlighting and optional context-aware completions. Use `ExpressionEditor` anywhere users need to write
expressions:

- Add it above a grid to build a [live filter](/docs/expression-filters) that users can type into.
- Embed it in a column configuration panel to let users define [computed fields](/docs/expression-fields).
- Use it in any form that accepts expressions.

::demo[Expression Editor with Completions="./demos/expression-completions"]

## Expression Editor Anatomy

`ExpressionEditor` is a compound component assembled from four parts. Omit the completion
sub-components entirely to use `ExpressionEditor` as a pure syntax-highlighted input.
...
with no children returns a fully functional syntax-highlighted input with no completion UI:
...
`ExpressionEditor.CompletionPopover` is a floating panel positioned at the cursor using the
native [Popover API](https://developer.mozilla.org/en-US/docs/Web/API/Popover_API). The component opens
and closes automatically based on completion state—you do not manage this behavior.

```tsx
<ExpressionEditor.Root ...>
  <ExpressionEditor.CompletionPopover className="border rounded-lg py-1">
    ...
  </ExpressionEditor.CompletionPopover>
</ExpressionEditor.Root>
```
...
`createCompletionProvider` builds a completion provider from a context object—the same
...
Completions open automatically in two situations or can be opened manually:

- **Dot** — typing `.` immediately triggers a property access completion.
- **Space** — typing a space after a complete value immediately triggers completions (useful for viewing available binary operators).
- **Manual** — pressing `Ctrl+Space` (or `⌘+Space` on macOS) opens completions at the current cursor position.
...
The provider can also return a `Promise<CompletionItem[]>` for async lookups such as
server-side field searches.
