...
Filtering the grid on every keystroke can be computationally expensive. Debounce the
filter expression input so the grid updates only after the user pauses typing.

Maintain two state values:

- `value`: The value shown in the editor.
- `committed`: The value used to compute the filter.

The `committed` state lags behind `value` while the user types,
updating to match it after the debounce delay.
...
Pass `value` to `ExpressionEditor.Root` and `committed` to the memoized filter.
The editor updates immediately, while the grid updates only after the user pauses typing.
