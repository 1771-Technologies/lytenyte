...
Recomputing the column on every keystroke can be computationally expensive.
Debounce the expression input so the column
updates only after the user pauses typing.
...
Pass `value` to `ExpressionEditor.Root` and `committed` to the AST Memo. The editor
updates immediately, while the grid updates only after the user pauses typing.
