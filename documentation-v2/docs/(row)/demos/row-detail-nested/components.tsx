...
export function Header({ column }: Grid.T.HeaderParams<GridSpec>) {
  return (
    <div className="text-ln-gray-60 group relative flex h-full w-full cursor-pointer items-center text-sm transition-colors">
...
const base: Grid.ColumnBase = {
  width: 100,
  widthFlex: 1,
  headerRenderer: (p) => <Header {...(p as any)} />,
  cellRenderer: (p) => {
    const field = p.api.columnField(p.column, p.row) as number;

    return (
      <div className="text-ln-gray-100 flex h-full w-full items-center text-sm tabular-nums">
        {field.toFixed(2)}%
      </div>
    );
  },
};
export function RowDetailRenderer({ row }: Grid.T.RowParams<GridSpec>) {
  ...
  return (
    <div className="flex h-full w-full items-center px-8">
      <div className="border-ln-gray-20 h-24 w-full overflow-hidden rounded-lg border">
        <Grid rowHeight="fill:24" columns={nestedColumns} rowSource={ds} columnBase={base} />
      </div>
    </div>
  );
}
