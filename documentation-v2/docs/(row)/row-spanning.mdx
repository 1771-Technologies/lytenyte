...
LyteNyte Grid uses a scan-based approach to optimize for correctness and performance.
Unlike grids that precompute spans (which limits scalability) or render spans inaccurately
when scrolling, LyteNyte Grid requires you to set a maximum span so it can maintain both
