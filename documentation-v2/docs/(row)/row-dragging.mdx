---
title: Row Dragging
step: Drag and drop rows within or between LyteNyte grids, or into external drop zones and apps.
...
The `getRowDragData` function is a helper provided by LyteNyte Grid.
...
- **String:** A CSS selector targeting the HTML element to serve as the placeholder.
  The browser captures a snapshot of this element to generate the placeholder image.
- **Query Object:** An object containing a **query** string and an optional **offset** tuple.
  This behaves like a string value but allows you to shift the placeholder
  image using the provided tuple.
- **Component Placeholder:** A custom React component that renders the placeholder.
  This option disables browser drag-image snapshots. Use this option only when drag
  operations stay within the same page.
