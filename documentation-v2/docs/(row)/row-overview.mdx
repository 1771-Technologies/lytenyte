...
- [`RowGroup`](/docs/reference/row): A row that contains child rows and establishes a hierarchical relationship.
- [`RowAggregated`](/docs/reference/row): A row that contains aggregate data but is not a group node that can be expanded.
- [`RowLeaf`](/docs/reference/row): A row with no child rows.
