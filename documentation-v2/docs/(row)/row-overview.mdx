...
- [`RowGroup`](/docs/reference): A row that contains child rows and establishes a hierarchical relationship.
- [`RowAggregated`](/docs/reference): A row that contains aggregate data but is not a group node that can be expanded.
- [`RowLeaf`](/docs/reference): A row with no child rows.
