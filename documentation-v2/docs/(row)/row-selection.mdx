...
in the grid using the header checkbox in the [marker column](/docs/marker-column). The demo
uses the [SelectAll component](/docs/component-select-all). However, you can achieve the same select-all
behavior programmatically with the [`api.rowSelect`](/docs/reference/grid-api) method, as shown below:
...
The code below shows how to set `linked` and `isolated` selection when using the [client row data source](/docs/client-source-overview).
...
In most cases, it is sufficient to call the [`rowsSelected`](/docs/reference/grid-api)
