...
The **Header Mode** has two variants, one variant renders only the top-level containers, and the fully rendered
variant renders every cell.
...
Each section component accepts an optional render prop as `children`, which allows more fine-grained control over
how rows and cells render. Like `Grid.Header`, this approach gives you two ways to render rows.

The default (and simplest) approach renders the row section components without providing a render prop, as shown
below. This structure matches the default mode, so it is equivalent to rendering `<Grid />`.
...
To take more control over row rendering, provide a render prop as `children` to a row section component.
