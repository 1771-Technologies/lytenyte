...
The render prop function receives an array of [`LayoutHeader`](/docs/reference/layout) items. Each `LayoutHeader` item describes
...
The render prop receives a [`LayoutRow`](/docs/reference/layout) item. Each `LayoutRow` describes the type of row to render, along
