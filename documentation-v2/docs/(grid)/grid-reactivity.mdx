...
always reflects the props you provide. The code below shows a basic example of
passing columns to the grid.

You should already be familiar with React, and passing props to components in this manner
...
The example below shows one case. See our [TypeScript guide](/docs/prodready-typescript) for more
details.
...
and `onRowDetailExpansionsChange` props to the grid. As demonstrated in the demo below:
...
(providing a change handler is optional).

For example, providing `rowDetailExpansions`
...
to update columns you must provide an `onColumnsChange` handler.
...
Avoid creating and passing objects in the render path of a component. A common
...
our [React Compiler guide](/docs/grid-react-compiler) to learn how to use LyteNyte Grid with
the new compiler.
