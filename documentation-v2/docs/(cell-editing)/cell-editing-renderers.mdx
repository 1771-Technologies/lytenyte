...
- There is no way to force the select element to open when the edit begins. Users must press the Space or
...
When creating a custom popover there are a few considerations you should be aware of that will impact the
popover's functionality:

- The **Enter** key is used by LyteNyte Grid to commit an edit. Combobox component usually use the **Enter**
  key to open the completion menu. Your code will have to explicitly make a choice on the behavior to use.
- The **ArrowDown** key will move focus to the next cell. Many select components use **ArrowDown** key to
  open the select options. Like the **Enter** key your code will need to make a choice on the behavior.
- The cell's in LyteNyte Grid will not allow content to overflow their bounds. This means a popover container
  will be clipped if it is rendered within the cell element. Most dialogs and popover use a React portal to
  render their content in the `body` element, which will resolve this issue.
- By default editing cells begins whenever any printable character is pressed over a cell. However, this behavior
  may be undesirable if you are using a selection component. You can disable this by setting the `editOnPrintable`
  property on the column being edited to `false`.
