...
test("should open the popover with the correct placements with arrow", async ({ page }) => {
  await page.goto(getUrl("with-arrow"));
  ...
  for (const placement of placements) {
    ...
    await expect(dialog).toBeVisible();
    await expect(page).toHaveScreenshot(`with-arrow-placement-${placement}.png`);
