...
description: >
  ...
  to CSV/Excel/Parquet/Arrow, row selection, theming or styling, TypeScript GridSpec patterns,
  server-side or tree data, and any PRO component (SmartSelect, PillManager, Menu, Dialog,
  TreeView, RowGroupCell). Also activate when the user describes grid problems without naming
  the package — e.g. "my rows won't group", "cells aren't editable", "add a loading overlay",
  "pin this column", "the filter isn't working", "how do I export this table".
...
- **No `useState` in cell renderers** — the grid virtualizes rows; cells unmount/remount as they scroll. State resets on unmount. Lift state into a React context or the `apiExtension`.
...
| Grid setup, container sizing, reactivity, API ref, `apiExtension`, `usePiece`, headless mode                                                        | [refs/grid-core.md](refs/grid-core.md)                                 |
...
| Cell renderers, tooltips, diff flashing, virtualization warning                                                                                     | [refs/cells.md](refs/cells.md)                                         |
