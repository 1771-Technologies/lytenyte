import { GitHubLogoIcon } from "@radix-ui/react-icons";
import { LyteNyteLogo } from "../ui/logos/lytenyte-logo";
import { Separator } from "../ui/separator";
import { ActiveLink } from "./active-link";

export function SecondaryHeader() {
  return (
    <div className="flex h-12 items-center gap-4">
      <div className="flex h-full gap-8 py-2">
        <LyteNyteLogo width={100} />
        <Separator />
      </div>
      <div className="h-full flex-1">
        <ul className="flex h-full items-center gap-2">
          <li className="h-full">
            <ActiveLink
              to="/lytenyte"
              match="/lytenyte"
              exclude={["/lytenyte/reference", "/lytenyte/changelog"]}
            >
              Guides
            </ActiveLink>
          </li>
          <li className="h-full">
            <ActiveLink to="/lytenyte/reference" match="/lytenyte/reference" exclude={[]}>
              API Reference
            </ActiveLink>
          </li>
          <li className="h-full">
            <ActiveLink to="/lytenyte/changelog" match="/lytenyte/changelog" exclude={[]}>
              Changelog
            </ActiveLink>
          </li>
          <li className="h-full">
            <ActiveLink
              to="https://github.com/1771-Technologies/lytenyte"
              className="flex items-center gap-2"
              target="_blank"
              match="https://github.com/1771-Technologies/lytenyte"
              exclude={[]}
            >
              GitHub
              <GitHubLogoIcon />
            </ActiveLink>
          </li>
        </ul>
      </div>
      <div className="flex items-center gap-4">
        <div>Open Source</div>
        <div>Help</div>
      </div>
    </div>
  );
}
