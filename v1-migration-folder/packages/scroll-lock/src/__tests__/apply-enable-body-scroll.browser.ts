...
test("applyEnableBodyScroll: decrements locksIndex and removes lock if index hits zero", () => {
...
test("applyEnableBodyScroll: retains lock if index stays above zero", () => {
  const el = document.createElement("div");
...
test("applyEnableBodyScroll: clears touch handlers and removes listener on iOS when no locks left", () => {
  const el = document.createElement("div");
  el.ontouchstart = () => {};
  el.ontouchmove = () => {};
...
test("applyEnableBodyScroll: calls applyPositionSetting when no locks remain on iOS", () => {
  const el = document.createElement("div");
...
test("applyEnableBodyScroll: calls applyRestoreOverflow when no locks remain on non-iOS", () => {
  const el = document.createElement("div");
