...
describe("applyDisableBodyScroll", () => {
  beforeEach(() => {
    // Reset globals
    bslGlobals.locks = [];
  ...
  test("adds lock and calls applyOverflowHidden on non-iOS", () => {
  ...
  test("adds lock and sets touch handlers + calls applyPositionFixed on iOS", () => {
    const el = document.createElement("div");
  ...
  test("attaches document-level touchmove listener only once on iOS", () => {
    const el = document.createElement("div");
    (isIOS as any).mockReturnValue(true);
    bslGlobals.documentListenerAdded = false;
  ...
  test("does not add duplicate lock if already locked", () => {
    const el = document.createElement("div");
    (isIOS as any).mockReturnValue(false);
  ...
  test("increments locksIndex count per element", () => {
    const el = document.createElement("div");
    (isIOS as any).mockReturnValue(false);
  ...
  test("ontouchstart handler sets initialClientY on iOS for single touch", () => {
    const el = document.createElement("div");
    (isIOS as any).mockReturnValue(true);
  ...
  test("ontouchmove handler calls handleScroll on iOS for single touch", () => {
    const el = document.createElement("div");
    (isIOS as any).mockReturnValue(true);
