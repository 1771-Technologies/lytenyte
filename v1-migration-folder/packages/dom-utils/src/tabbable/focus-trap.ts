...
import { getActiveElement, getDocument, isDocument } from "./node.js";
import { getFocusables, getTabbables, isFocusable, isTabbable } from "./tabbable.js";
        ...
        if (getTabIndex(this.state.mostRecentlyFocusedNode) > 0) {
          ...
          if (
            !this.state.containerGroups.some((g) =>
              g.tabbableNodes.some((n) => getTabIndex(n as HTMLElement) > 0),
            )
          ) {
