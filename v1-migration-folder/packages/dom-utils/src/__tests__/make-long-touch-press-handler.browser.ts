...
test("makeLongTouchPressHandler: fires callback after long touch", async () => {
  const spy = vi.fn();
...
test("makeLongTouchPressHandler: does not fire if touchend happens early", async () => {
  const spy = vi.fn();
...
test("makeLongTouchPressHandler: does not fire if touchcancel happens", async () => {
  const spy = vi.fn();
  const handler = makeLongTouchPressHandler(spy, 100);
...
test("makeLongTouchPressHandler: does not fire if touch moves more than 20px", async () => {
  const spy = vi.fn();
  const handler = makeLongTouchPressHandler(spy, 100);
...
test("makeLongTouchPressHandler: ignores if multiple touches are detected", async () => {
  const spy = vi.fn();
  const handler = makeLongTouchPressHandler(spy, 50);
