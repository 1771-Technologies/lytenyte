...
test("isTabbableRadio: returns false when querySelectorAll fails due to invalid name and missing CSS.escape", () => {
...
test("isTabbableRadio: returns true for radio in shadow DOM without a form", () => {
...
test("isTabbableRadio: returns true for one of multiple radios with same name but no form", () => {
...
test("isTabbableRadio: returns true for checked radio in shadow DOM", () => {
  const host = document.createElement("div");
  document.body.appendChild(host);
...
test("isTabbableRadio: radios with same name in separate shadow roots are independently tabbable", () => {
