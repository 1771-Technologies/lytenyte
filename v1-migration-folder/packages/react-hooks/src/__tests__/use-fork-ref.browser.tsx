...
test("useForkRef: should correctly merge refs", async () => {
  const fn = vi.fn();
  const fn2 = vi.fn();
  const Comp = () => {
...
test("useForkRef: supports only function refs", () => {
  const fn = vi.fn();
  const Comp = () => {
...
test("useForkRef: updates object ref on unmount", async () => {
...
test("useForkRef: skips when all refs are undefined", () => {
  const Comp = () => {
