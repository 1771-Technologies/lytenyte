...
data interface—the protocol LyteNyte Grid uses to exchange data.
...
[API reference](/docs/reference/row-data-source#datarequestmode) for details on each property.
...
- [Pivoting](/docs/server-data-loading-pivoting): walks through implementing server-side column
  pivoting and highlights important considerations.
