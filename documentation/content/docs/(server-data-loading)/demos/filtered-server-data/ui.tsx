...
>(({ align = "center", sideOffset = 4, ...props }, ref) => (
  <PopoverPrimitive.Portal>
    <PopoverPrimitive.Content
      ...
      side="bottom"
      className={
        "bg-ln-gray-10 text-ln-gray-80 data-[state=closed]:animate-popover-out data-[state=open]:animate-popover-in z-50 max-w-[98vw] min-w-[240px] origin-(--radix-popover-content-transform-origin) rounded-xl border p-2 text-sm shadow-lg backdrop-blur-lg focus-visible:outline-none"
      }
      {...props}
    />
  </PopoverPrimitive.Portal>
    ...
    >
      <Select.Trigger
        ...
        className={tw(
          ...
          "overflow-hidden text-nowrap text-ellipsis whitespace-nowrap",
          p.className,
        )}
      >
      ...
      <Select.Portal>
        <Select.Content
          position="popper"
          sideOffset={5}
          className="border-ln-gray-30 bg-ln-gray-02 z-[100] max-h-[300px] min-w-[var(--radix-select-trigger-width)] overflow-x-hidden overflow-y-auto rounded-lg border shadow-[0_14px_18px_-6px_rgba(30,30,41,0.07),0_3px_13px_0_rgba(30,30,41,0.10)] md:max-h-[unset]"
          inert={false}
        >
    ...
    return (
      <Select.Item
        className={tw(
          ...
          "relative flex cursor-pointer items-center leading-none select-none",
          className,
        )}
        {...props}
