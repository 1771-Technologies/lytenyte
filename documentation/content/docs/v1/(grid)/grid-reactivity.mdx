...
the [API reference](/docs/v1/reference/grid-atom) for a more technical explanation.
...
function RenderSelectionCount({ rowSelectedIds }: { rowSelectedIds: GridAtom<Set<string>> }) {
  ...
  return <div className="border-b px-2 py-2">There are {selectedRows.size} selected rows in the grid.</div>;
}
```
...
- [Row & Column Virtualization](/docs/v1/grid-virtualization): Learn how LyteNyte Grid enables performance at scale with row and column virtualization.
