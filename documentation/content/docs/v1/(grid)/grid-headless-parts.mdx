...
see the [Getting Started guide](/docs/v1/intro-getting-started).
...
[grid state object](/docs/v1/reference/grid-state#grid) as a prop. Create this grid state object using
...
[Responsive Container guide](/docs/v1/grid-container) for details on configuring containers
for the grid.
...
[grid state object](/docs/v1/reference/grid-state#gridstate).
...
The `Grid.useLyteNyte` hook returns a grid view [atom](/docs/v1/grid-reactivity). You can use the grid view atom
...
[`HeaderGroupCellLayout`](/docs/v1/reference/grid-view#headergroupcelllayout) API reference.
...
For more information, see the [Column Header Renderer](/docs/v1/column-header-renderer) and
[Column Floating Header](/docs/v1/column-floating-header) guides respectively.
...
function RowSection<D = any>({
  ...
  const Section = section === "top" ? Grid.RowsTop : section === "bottom" ? Grid.RowsBottom : Grid.RowsCenter;
...
For more details, see the [Row Full Width guide](/docs/v1/row-full-width).
...
- [Responsive Container](/docs/v1/grid-container): Learn how to configure containers for LyteNyte Grid.
