...
Parameters provided to the [RowFullWidthPredicate](/docs/v1/reference/row#rowfullwidthpredicate) function to determine
if a row should span full width of the grid.

<AutoTypeTable path="./types.ts" name="RowFullWidthPredicateParams" />
...
<AutoTypeTable path="./types.ts" name="RowFullWidthRendererParams" />
...
<AutoTypeTable path="./types.ts" name="RowGroup" />
...
<AutoTypeTable path="./types.ts" name="RowLeaf" />
...
<TypeTable
  type={{
    RowNode: {
      description:
        "A union of  [RowLeaf](/docs/v1/reference/row#rowleaf)  and  [RowGroup](/docs/v1/reference/row#rowgroup) . Represents any row that may appear in the grid view.\nUsed generically when the row type is not known ahead of time.",
      type: "RowLeaf | RowGroup",
    },
  }}
/>
