...
const columns = [
  {
    id: "symbol",
    autosizeHeaderFn: (p) => {
      const textWidth = measureText(`${p.column.name ?? p.column.id}`, p.grid.state.viewport.get()).width;
...
see the [Row & Column Virtualization guide](/docs/v1/grid-virtualization).
...
- [Row and Column Virtualization](/docs/v1/grid-virtualization): Learn how LyteNyte Grid enables performance at scale with row and column virtualization.
