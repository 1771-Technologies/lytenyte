...
export default function ColumnBase() {
  ...
  useEffect(() => {
    if (height === "small") grid.state.headerHeight.set(24);
    if (height === "medium") grid.state.headerHeight.set(40);
    if (height === "large") grid.state.headerHeight.set(60);
  }, [grid.state.headerHeight, height]);

  return (
    <div>
      <div className={tw("flex h-full items-center gap-1 text-nowrap border-b px-2 py-2")}>
        <div className={tw("text-light hidden text-xs font-medium md:block")}>Header Height:</div>
        <ToggleGroup
          type="single"
          value={height}
