...
Adds LyteNyte Grid to the [shadcn registry](https://ui.shadcn.com/docs/v1/directory). See the
[installation guide](/docs/v1/intro-installation-shadcn) to get started with LyteNyte Grid in shadcn.
...
[theming guide](/docs/v1/grid-theming) for instructions on using the prebuilt themes.
...
- `requestForGroup`: Returns the [DataRequest](/docs/v1/reference/row-data-source#datarequest) for a given group row.
- `requestForNextSlice`: Returns the [DataRequest](/docs/v1/reference/row-data-source#datarequest) for the next slice to aid optimistic data fetching.
- `requestsForView`: Returns the [DataRequests](/docs/v1/reference/row-data-source#datarequest) for the current view.
...
See the [server data loading guides](/docs/v1/server-data-loading-overview) for full details on using the server data source
in LyteNyte Grid.
...
- Editing props consolidated. `cellEditPredicate`, `cellEditParser`, `cellEditUnparser`, `cellEditProvider`, `cellEditParams`, and `cellEditRowUpdater`
  are replaced by `editRenderer`, `editSetter`, and `editable`. See our [cell editing guide](/docs/v1/cell-editing).
...
- Pivot-related properties refactored. `columnPivotModel` now holds the complete pivot configuration. See the [column pivoting guide](/docs/v1/column-pivoting).
