...
- [Filter Model](/docs/v1/reference/filters#filtermodelitem): Standard column filters that
  apply predicates to cell values, such as greater than or less than comparisons.
- [Filter In Model](/docs/v1/reference/filters#filterin): A tree-set definition used
  for inclusion or exclusion of values.
- [Quick Search](/docs/v1/filter-quick-filter): A string-based search that matches a value across any cell in a row.
...
[`DataInFilterItemFetcherParams`](/docs/v1/reference/row-data-source#datainfilteritemfetcherparams) interface:
...
The fetcher should return an array (or `Promise`) of [`FilterInFilterItem`](/docs/v1/reference/filters#filterinfilteritem) objects:
...
The example below uses LyteNyte Grid's [Filter Tree](/docs/v1/component-filter-tree)
...
- [Server Row Grouping and Aggregations](/docs/v1/server-data-loading-row-grouping-and-aggregation): Learn how
  to fetch grouped data and handle row expansion and collapse.
- [Server Row Sorting](/docs/v1/server-data-loading-row-sorting): Implement and display server-side sorting.
- [Optimistic Loading](/docs/v1/server-data-loading-optimistic-loading): Use optimistic loading
