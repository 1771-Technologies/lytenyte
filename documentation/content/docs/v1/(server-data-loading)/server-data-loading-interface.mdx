...
overview. For a practical guide, see the [Server Row Data guide](/docs/v1/server-data-loading-row-data).

## The Request Interface

At the core of the server data interface is the request interface, the API description of which
may be found in the [API reference](/docs/v1/reference/row-data-source#datarequest). In plain
...
- A data response for scrollable rows, described in this
  [API reference](/docs/v1/reference/row-data-source#dataresponse).
- A data response for pinned rows (top or bottom), described in this
  [API reference](/docs/v1/reference/row-data-source#dataresponsepinned).
...
[API reference](/docs/v1/reference/row-data-source#dataresponseleafitem)). Its TypeScript definition is
shown below:
...
The `id` value maps to the [row node's](/docs/v1/reference/row#rowleaf) `id` property, and the `data`
...
items to create [row group nodes](/docs/v1/reference/row#rowgroup). Group nodes represent branches in
...
[API reference](/docs/v1/reference/row-data-source#dataresponsebranchitem).
...
[row group node](/docs/v1/reference/row#rowgroup) is intentional. The server response is designed to
...
([API reference](/docs/v1/reference/row-data-source#dataresponsepinned)) defines the response type that
...
const ds = useServerDataSource<MyDataType>({
  dataFetcher: (params: DataFetcherParams<MyDataType>): Promise<(DataResponse | DataResponsePinned)[]> => {
...
([API reference](/docs/v1/reference/row-data-source#datafetcherparams)), whose TypeScript definition is
shown below:
...
[API reference](/docs/v1/reference/row-data-source#datarequestmodel) for details on each property.
...
- [Server Row Data](/docs/v1/server-data-loading-row-data): Shows how to fetch server data in slices.
- [Server Row Sorting](/docs/v1/server-data-loading-row-sorting): Explains server-side sorting and how it is
  displayed client-side.
- [Server Row Filtering](/docs/v1/server-data-loading-row-filtering): Covers server-side filtering, including
  advanced features such as **in filters (tree set filters)** and **quick search filters**.
- [Optimistic Loading](/docs/v1/server-data-loading-optimistic-loading): Explores LyteNyte Grid's
