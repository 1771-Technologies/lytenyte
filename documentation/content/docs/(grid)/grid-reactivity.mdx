...
A **grid atom** is the core primitive of LyteNyte Grid's state system. A grid atom works alongside
React's state primitives and provides a consistent way to retrieve, update, and reactively consume state.
Most properties on the `state` store are grid atoms.

Grid atoms come in two variants: readonly atoms and writable atoms. Readonly atoms represent derived
state, which updates when writable atoms change. Writable atoms hold values that you can directly update.
Both variants provide methods that React can use to re-render when atom values change. The subsections
below cover the atom methods. You can also refer to the
[API reference](/docs/reference/grid-atom) for a more technical explanation.
...
When passing grid atoms around, pass the atom itself rather than its resolved value. Call `useValue` as
...
LyteNyte Grid batches multiple updates into a single `watch` callback. For example, calling multiple subsequent calls to `set`:
