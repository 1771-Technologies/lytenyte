...
The following sections explain the individual parts of the grid component anatomy and then end with
a full working example. This section provides a high-level view of the structure:
...
Render `Grid.HeaderRow` components inside `Grid.Header`. There is always at least one header row, but you
may have more depending on the column definitions. The total number of header rows equals the maximum column
group depth, plus one for the floating row (if enabled), plus one for the column header row.

The `Grid.useLyteNyte` hook returns a grid view [atom](/docs/grid-reactivity). You can use the grid view atom
to obtain layout state for the current view. The header layout includes an array of header rows. Render a
`Grid.HeaderRow` for each header row in the layout. The code below shows this:
...
its corresponding index through the `headerRowIndex` prop. LyteNyte Grid uses this index to position
...
render a `Grid.HeaderGroupCell`. Otherwise, render a `Grid.HeaderCell`. In code:
...
In contrast, `Grid.HeaderCell` does not accept `children`. Instead, the content comes from the `cellRenderer`
property on the column the cell belongs to, or from the `floatingCellRenderer` property when the header cell
is part of the floating row. For more information, see the
[Column Header Renderer](/docs/column-header-renderer) and
[Column Floating Header](/docs/column-floating-header) guides.

## Rows Container

Use the `Grid.RowsContainer` component to render all grid rows. `Grid.RowsContainer` plays a similar role
to `Grid.Header`: it hosts the rows of the grid. Like `Grid.Header`, `Grid.RowsContainer` renders a normal
`div` element and accepts all `div` props. Updating the example, we now have:
...
- A normal grid row, which has one cell per column and renders with `Grid.Row`.
- A full width row, which renders a single cell that spans the viewport width and renders with `Grid.RowFullWidth`.

Earlier in this guide you used the grid's `view` state to determine the header layout. Rows use the same
`view` state, but the layout splits into top, center, and bottom sections. You access the layout for each
section through `view.rows`. For example, `view.rows.center` contains the center rows. For the center
...
When a row contains cells (all rows except full width rows), the `cells` property on the row is an array of
cells that you render using `Grid.Cell`. Updating the `RowSection` component gives us:
