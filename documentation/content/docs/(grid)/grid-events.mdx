...
LyteNyte Grid defines its own events that fire on specific user interactions. This guide
explains how to listen for and handle these events. It does not cover every event the grid
can fire. Refer to the [Events API reference](/docs/reference/events) for details on
individual events.

### Listening to Events

The simplest way to listen to events is to add an event listener to the `Grid.Root` component.
For example, the demo below alerts the user every time a row is selected.

!demo:Row Selection Event="./demos/grid-add-event-listener"

The code for handling the event is straightforward and should feel familiar to React developers:
...
In the code above, the row selection event is named `onRowSelectEnd`. Many LyteNyte Grid events
are split into two or more phases because some actions may be asynchronous. For row selection,
LyteNyte Grid also fires an `onRowSelectBegin` event when the user attempts to select a row.
...
when the component that creates state is different from the component that renders the grid. The
example below demonstrates imperatively adding an event listener:

!demo:Imperative Event Listener="./demos/grid-imperative-event"

This example behaves the same as the previous one (it alerts the user whenever a row is selected),
but it attaches the listener inside a `useEffect`:
...
If you review the list of events in the
[API reference](/docs/reference/events), you will notice that all of them are tied to grid-specific
interactions. LyteNyte Grid does not define events such as cell-clicked events or scroll events.
This design follows the headless component architecture.
