...
function PriceCellImpl({ grid, row, column }: CellRendererParams<StockData>) {
  const field = grid.api.columnField(column, row);

  const label = typeof field === "number" ? formatter.format(field) : "-";
  const labelGBP = typeof field === "number" ? formatter.format(field * 1.36) : "-";
  ...
  return (
    <div className="flex h-full w-full items-center justify-end gap-1 text-nowrap px-3 tabular-nums">
      <div className="flex items-baseline gap-1">
        <span>{label}</span>
        {!isCount && <span className="text-[9px]">USD</span>}
      </div>
      <span>/</span>
      <div className="flex items-baseline gap-1">
        <span>{labelGBP}</span>
        {!isCount && <span className="text-[9px]">GBP</span>}
        @1.36
      </div>
    </div>
  );
