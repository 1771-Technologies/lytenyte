...
export function HeaderCell({ column, grid }: HeaderCellRendererParams<OrderData>) {
  ...
  return (
    ...
    >
      {isUpdating && <HeaderCellInput name={column.name ?? column.id} onUpdate={onUpdate} />}
      {!isUpdating && <>{column.name ?? column.id}</>}
    </div>
  );
}

function HeaderCellInput({
  name,
  onUpdate,
  ...props
}: JSX.IntrinsicElements["input"] & { name: string; onUpdate: (newName: string) => void }) {
  const [value, setValue] = useState(name);

  return (
    <input
      className="bg-ln-gray-10 h-full w-full px-3 outline-none"
      value={value}
      onChange={(e) => setValue(e.target.value)}
      autoFocus
      onKeyDown={(e) => {
        if (e.key === "Escape") {
          // Try focus the header cell. This assumes only two levels of nesting.
