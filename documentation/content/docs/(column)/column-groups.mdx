...
Columns remain a flat array, even when grouped. LyteNyte Grid creates
...
LyteNyte Grid identifies column groups using the `groupPath` property.
A group appears split when columns from another group appear between its
members. Although the group remains a single logical entity, it renders
as multiple visual segments. When you move separated columns next to
each other, the grid merges the segments automatically. Pinned columns
always appear visually separate from non-pinned columns, even when they
share a group.
...
!demo:Column Groups Split="./demos/column-groups-split_x"
...
the same `id`. Because React requires unique keys, the grid provides
...
!demo:Column Groups With Pinned="./demos/column-groups-expansions-pinned_x"
...
Collapsible groups expand by default. Use the
`columnGroupDefaultExpansion` property to change this. Setting it to
`false` collapses all groups initially:
...
horizontally and observe that the **Market Info** label remain in view.
