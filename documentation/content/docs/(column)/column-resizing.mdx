...
The demo below enables the drag handle. Try placing your cursor on the right edge
of a column's header and dragging once the hover indicator is shown.
...
It accepts an object mapping column IDs to widths.
...
You can set `width` to a value outside the `widthMin` and `widthMax` bounds.
The grid clamps the value during layout. If you later change the bounds,
the column's visible width may adjust to match the new limits. For example, the
column configuration below is perfectly valid. Adjusting the `widthMax` property to
a higher value will result in the column immediately resizing to that higher value, until
the column has reached its desired width value of `3000px`.
...
!demo:Filling Free Space="./demos/column-resizing-width-flex_x"
...
!demo:Size Columns To Fit="./demos/column-resizing-size-to-fit_x"

Use size-to-fit sparingly. Columns can appear compressed on narrow viewports,
and cell content may truncate. Only enable this feature when all columns must be visible.

## The Measure Text Function
...
For example, applications like Excel show `#` symbols when a number overflows the cell. You can
replicate this behavior in LyteNyte Grid by comparing the measured text width with the cell width.
The demo below illustrates the pattern.

!demo:Hash Truncate Cells="./demos/column-resizing-measure-text_x"
