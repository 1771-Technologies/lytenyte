...
The demo below shows single-row selection configured to select rows on a single click.
Other configurations can be used to support multiple selection modes
or checkbox-based selection, which are covered in later sections.
...
This behavior uses the [`grid.api.rowSelectAll`](/docs/reference/grid-api#gridapi) method.
...
In the demo below, the grid prevents selection of rows with
