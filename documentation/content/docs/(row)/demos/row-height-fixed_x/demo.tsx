...
export default function RowHeight() {
  ...
  useEffect(() => {
    ...
    if (height === "medium") grid.state.rowHeight.set(40);
    if (height === "large") grid.state.rowHeight.set(60);
  }, [grid.state.rowHeight, height]);

  return (
    <>
      <div className={tw("flex h-full items-center gap-1 text-nowrap border-b px-2 py-2")}>
        <div className={tw("text-light hidden text-xs font-medium md:block")}>Row Height:</div>
        <ToggleGroup
          type="single"
          value={height}
