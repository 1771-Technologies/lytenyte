...
import { type DocEntry, type GeneratedDoc, type GenerateOptions, type Generator } from "./base";
...
export function generateMDX(
  ...
  return source.replace(regex, (...args) => {
    ...
    const docs = generator.generateDocumentation({ path: file }, groups.name, rest);

    return docs
      .map((doc) => templates.block(doc, doc.entries.map(templates.property).join("\n")))
      .join("\n\n");
  });
}
